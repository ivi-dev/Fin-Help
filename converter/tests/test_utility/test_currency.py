...
class TestCurrencyUtility(TestCase):
	raw_data_element = f"""\
		...
		<body>
			<{DATA_ELEMENT_NAME} id="{DATA_ELEMENT_ID}">
				<h2>Курсове на българския лев към отделни чуждестранни валути и цена на златото, валидни за {date_valid[0]}.{date_valid[1]}.{date_valid[2]}</h2>
                    <div class="table_box">
	...
	def create_currency_data_list(self):
	...
	def test_update_currency_data(self, mock_get):
		...
		Currency.objects.create(name='Currency 1',
								code='ABC',
								per=1,
								rate=12.12345)
		Currency.objects.create(name='Currency 2',
								code='DEF',
								per=1,
								rate=54.54321)
		...
		self.assertEqual(new.rate, round(Decimal(currency_data2['rate']), 
										 Currency.precision))
		self.assertEqual(new.date_valid, datetime.date(int(date_valid[2]), 
													   int(date_valid[1]), 
												       int(date_valid[0])))
...
class MockResponse:
	def __init__(self):
		self.text = TestCurrencyUtility.raw_data_element